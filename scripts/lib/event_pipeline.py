...
This module is intentionally conservative in v0.1:
- it does not replace the full generate_biweekly_report.py logic yet;
- it provides reusable wrapper functions around the existing lib modules;
- it makes future migration easier by giving the main script one stable pipeline API.

Planned use:
    from lib.event_pipeline import prepare_event_cluster, prepare_event_clusters

The module is safe to add before wiring it into the workflow.
"""
...
try:
    from lib.event_scoring import calculate_cluster_score, cluster_sort_score
except Exception:
...
def normalize_cluster_location(cluster: Dict[str, Any]) -> Dict[str, Any]:
    """
    Add normalized_location to a cluster if the location_normalizer module exists.
    """
...
def validate_cluster_sources(
    ...
    *,
    ...
    """
    Add source_validation and source_confidence to a cluster.
    """
...
def is_cluster_source_valid(cluster: Dict[str, Any]) -> bool:
    """
    Return whether the cluster has enough source evidence.
    """
...
def build_cluster_context(clusters: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Build lightweight counters used by event scoring and ranking.
    """
...
def score_cluster(
    ...
    *,
    ...
    """
    Add score and ranking_breakdown-compatible information to a cluster.

    If event_scoring.py is available, use it. Otherwise keep the existing score.
    """
    ...
    if calculate_cluster_score is None:
        ...
        cluster.setdefault("ranking_breakdown", {
            "total": cluster["score"],
    ...
    # Keep any richer breakdown already produced by the main script.
    cluster.setdefault("ranking_breakdown", {
        "total": cluster["score"],
...
def final_validate_cluster(
    ...
    *,
    ...
    """
    Add final_validation to a cluster.

    By default fetch_articles=False for safe pipeline migration. The main script
    can enable article fetching only after we have measured runtime.
    """

    cluster = dict(cluster or {})

    if validate_final_event is None:
        cluster.setdefault("final_validation", {
            "status": "unavailable",
    ...
    # Do not overwrite score yet. Store recommended score only.
...
def prepare_event_cluster(
    ...
    *,
    ...
    """
    Full preparation chain for one selected or candidate event cluster.

    Steps:
    1. normalize location
    2. source validation
    3. scoring
    4. optional final validation
    """
...
def prepare_event_clusters(
    ...
    *,
    ...
    This is the main migration-friendly entry point.
    """
...
def filter_valid_clusters(clusters: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Keep only source-valid clusters.
    """
...
def sort_clusters(
    ...
    *,
    ...
    """
    Sort clusters by the scoring module if available, otherwise by score.
    """
...
def select_top_clusters(
    ...
    *,
    ...
    """
    Select top clusters with simple diversity protection.

    This can later replace the selection logic inside generate_biweekly_report.py.
    """
...
def pipeline_summary(clusters: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Return lightweight QA statistics for prepared clusters.
    """
...
if __name__ == "__main__":
    ...
    sample = [{
        "date": "2026-06-30",
        "country": "Ukraine",
        "location": "Crimea, Crimea, Ukraine",
        "event_type": "Rajtaütés / fegyveres támadás",
        "event_nature": "Egyéb biztonsági esemény",
        "title": "Rajtaütés / fegyveres támadás – Crimea, Crimea, Ukraine",
        "sources": [
            "https://www.dw.com/en/ukraine-says-major-crimea-bridge-destroyed-in-latest-attack/a-77678698"
        ],
    }]

    prepared = prepare_event_clusters(sample, run_final_validation=True, fetch_articles=False)
    print(json.dumps({
        "prepared": prepared,
